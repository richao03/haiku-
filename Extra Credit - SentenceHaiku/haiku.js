...
function createHaiku(structure){ 
  var verse = [];
  var i = 0
  if (verse.length === structure.length){
    return verse
  }
  for (var j = 0; j< structure.length; j++) {
      if(answerArray[i][2]+answerArray[i+1][2]+answerArray[i+2][2] === structure [j]){
          verse.push(answerArray[i][0]+" "+answerArray[i+1][0]+" "+answerArray[i+2][0])
    } else {
      i++
